        ...
        {pageData.length === 0 ? (
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {pageData.map((b) => (
              <article
                key={b.id}
                className="rounded-md border border-neutral-200 p-4 hover:border-blue-400 hover:shadow-md transition-all cursor-pointer"
                aria-labelledby={`broker-${b.id}`}
                onClick={() => window.location.href = `/brokers/${b.id}`}
              >
                ...
                <p className="mt-3 text-xs text-blue-600 font-medium">View Profile →</p>
              </article>
            ))}
          </div>
        )}
