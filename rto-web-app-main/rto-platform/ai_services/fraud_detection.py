...
class FraudDetector:
    ...
    def detect_ghosting(
        ...
        if isinstance(otp_start_time, str):
            try:
                otp_start_time = datetime.fromisoformat(otp_start_time)
            except ValueError:
                return {'is_ghosting': False, 'confidence': 0.0, 'reason': 'Invalid OTP start timestamp'}

        if otp_close_time and isinstance(otp_close_time, str):
            try:
                otp_close_time = datetime.fromisoformat(otp_close_time)
            except ValueError:
                otp_close_time = None
    ...
    def detect_duplicate_submission(
        ...
        for existing in existing_applications:
            similarity = self._calculate_application_similarity(application_data, existing)

            if similarity >= similarity_threshold:
                duplicates.append({
                    'application_id': existing.get('id'),
                    'similarity': round(similarity, 3),
                    'matching_fields': self._get_matching_fields(application_data, existing)
                })

        is_duplicate = len(duplicates) > 0

        top_similarity = max([d['similarity'] for d in duplicates]) if duplicates else 0.0
    ...
    def detect_fee_inflation(
        ...
        threshold_pct = inflation_threshold * 100
        ...
        return {
            ...
            'reason': f"Fee {deviation_pct:.1f}% {'above' if deviation > 0 else 'below'} expected"
        }
    ...
    def comprehensive_fraud_check(
        ...
        fee_inflation = self.detect_fee_inflation(actual_fee, expected_fee=expected_fee)
