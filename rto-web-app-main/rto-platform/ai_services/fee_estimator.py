...
class FeeEstimator:
    ...
    def __init__(self):
        ...
        self.broker_fee_percent = {
            'Gold': 0.25,      # 25% of govt fee (premium tier)
            'Silver': 0.20,    # 20% of govt fee
            'Bronze': 0.15,    # 15% of govt fee (entry tier)
    ...
    def detect_fee_inflation(
        ...
        broker_tier: str = 'Silver',
        threshold: float = 0.25,  # 25% deviation threshold
        ...
        Backward compatibility:
            - When `expected_fee` is provided without service details, returns (is_inflated, deviation).
            - Otherwise returns detailed dictionary used by API layer.
        """
        if expected_fee is None:
            if service_type is None or vehicle_class is None:
                raise ValueError("service_type and vehicle_class required when expected_fee is not supplied")
        ...
        deviation = (actual_fee - expected_total) / expected_total if expected_total > 0 else 0.0
        is_inflated = deviation > threshold

        inflation_penalty = 0.0
        status = 'normal'
        if is_inflated:
            inflation_penalty = min(deviation * 2, 1.0)  # Cap penalty
            status = 'inflated'
        ...
        details = {
            ...
            'expected_fee': round(expected_total, 2),
        ...
        if expected_fee is not None and service_type is None and vehicle_class is None:
            return is_inflated, deviation
